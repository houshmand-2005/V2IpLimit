...
# یک فورک بروز شده از آیپی لیمیت مرزبان

# قابلیت ها 
- رفع مشکل صفر شدن لاگ ارسال برای ادمین تلگرام
- رفع مشکل عدم شمارش آیپی های  متصل به هر کانفیگ
- امکان  معافیت کانفیگ های دارای نام خاص از محدودیت
- محدود کردن هر کانفیگ بعد از سه بار اخطار
- ربات دوم جهت ارسال اخطار هم زمان به کاربر متخلف(کاربر متخلف باید در ربات دوم حتما عضو باشد )
- رفع مشل عدم ارسال کامل لاگ در ربات تلگرام
- ارسال لاگ info xray به ربات تلگرام 
- رفع باگ
...
```bash
bash /root/V2IpLimit/restart_v2iplimit.sh
```
# توجه 
...
**در فایل second_bot_start_listener.py توکن و چت آیدی ادمین ربات را تنظیم کنید ( ربات ارسال هشدار به کاربران متخلف)**

نمونه پیام :
```
⚠️ اکانت * توسط تعداد بیشتری از کاربر یا دستگاه نسبت به تعداد مجاز، در حال استفاده است.
به همین دلیل، اخطار 2/3 برای این اکانت ثبت شده. اگه این روند ادامه پیدا کنه، اکانت به‌صورت خودکار غیرفعال می‌شه.

🔸 توجه: اگر فکر می‌کنی این پیام اشتباهی برات ارسال شده، ممکنه به‌خاطر تعویض اینترنت یا اتصال موقت از چند جا باشه. در این صورت اخطار رو نادیده بگیر؛ سیستم به‌صورت خودکار اگر شرایط نرمال بشه، اخطارها رو پاک می‌کنه.

🔹 خواهشاً به پشتیبانی پیام نده — چون این پیام‌ها خودکار ارسال می‌شن و پشتیبانی  به اکانت‌های غیرفعال‌شده به هیچ عنوان رسیدگی نمیکنهه 🙏
